...
## Following steps happen after RNA-seq data has been aligned using STAR/Kallisto and correct sample IDs to sequencing IDs have been assigned


### Use the DESeq2 package in R to find DEGs, tidyverse to wrangle data and ggplot for visualization
...
* Load appropriate sample STAR aligned data (all of the Area X & Medial Striatum data have been provided)
  + For example Vignette, just male vehicle (MalVeh) Area X/medial striatum is shown
    * Results from females and other treatments are provided for the plotting portion of Vignette
...
MalVeh_X_STARcounts <- read.table("Vignette_materials/Counts_MV_mstX_STAR.tsv")

```


### Make matrix for labels and gene names prior to running DESeq2
* See DESeq2 documentation for specific directions on how to make the label matrix
  + This example uses the terms "Sex#_Tx_Surr" & "Sex#_Tx_Nuc" for surround and nucleus.
  + It is useful to specify sex, treatment, and the actual nucleus for downstream applications to avoid confusion.
    * e.g. "Mal1_Veh_LAi" & "Mal1_Veh_RA"
  + I also recommend changing the Sample ID's in the count matrix to match the labels
* A much larger label matrix accounting for all samples is needed when creating PCA plot/distance matrix. 
...
* Change the "ensembl_gene_id" to different category if using different annotation methods
  + Gene ID
  + Transcript ID
  + HGNC ID
  + etc. 
* Double check these, on 11/27/2019 there were some problems where complete genes were erased. This seems to have been resolved as of 11/30/2020.
* Choe2021 used ensembl data from an older assembly (TaeGut3.2.4: Release 98)
  + Older "mart", "TaeGutEnsembl" and "genelist" have been provided as .rds objects.
  + These will likely be obsolete by the time anyone uses this vignette, but has provided in case it is helpful
* If you run the Vignette as is, it will likely use the new assembly (possibly bTaeGut1_v1.p).
...
* Run DESeq2 on an appropriate count data matrix ("Output_of_aggregation") to output results as a data frame and a dds object
  ...
  + Run two sets of DE comparisons:
    ...
    * Pairwise comparisons of area within subject for individual comparisons. Less stringent between group comparisons
      + design = ~Subject + Area
  + Repeat for other alignment methods (if using more than 1 alignment method for cross validation)
    * STAR (Shown here)
    * Kallisto (previously run Kallisto results have been included as .tsv files in vignette materials)
...
DEG_MalE2_X_STAR <- read_tsv("Vignette_materials/DEG_MalE2_X_STAR.tsv")
DEG_MalExem_X_STAR <- read_tsv("Vignette_materials/DEG_MalExem_X_STAR.tsv")
DEG_FemVeh_X_STAR <- read_tsv("Vignette_materials/DEG_FemVeh_X_STAR.tsv")
DEG_FemE2_X_STAR <- read_tsv("Vignette_materials/DEG_FemE2_X_STAR.tsv")
DEG_FemExem_X_STAR <- read_tsv("Vignette_materials/DEG_FemExem_X_STAR.tsv")

DEG_MalVeh_X_Kallisto <- read_tsv("Vignette_materials/DEG_MalVeh_X_Kallisto.tsv")
DEG_MalE2_X_Kallisto <- read_tsv("Vignette_materials/DEG_MalE2_X_Kallisto.tsv")
DEG_MalExem_X_kallisto <- read_tsv("Vignette_materials/DEG_MalExem_X_Kallisto.tsv")
DEG_FemVeh_X_Kallisto <- read_tsv("Vignette_materials/DEG_FemVeh_X_Kallisto.tsv")
DEG_FemE2_X_Kalliso <- read_tsv("Vignette_materials/DEG_FemE2_X_Kallisto.tsv")
DEG_FemExem_X_Kallisto <- read_tsv("Vignette_materials/DEG_FemExem_X_Kallisto.tsv")

```


### Combine DESeq2 results into a list of data frames, organized by brain region
* Each list is made of the results data frame
* This is used to filter out and keep DEGs that pass a cutoff (FDR <0.5) 
* This Vignette will include examples from STAR and Kallisto:
...
* The significance filtering function will also pull the log2FC and FDR/pAdj from the STAR aligned data. 
  + These are not useful for making the common genes list, but will be used later for generating plots.
...
### If multiple alignment methods were used to cross-validate results, you will need to keep common genes
* You only need to keep genes that are common between the STAR and Kallisto lists with log2FC and pAdj values from preferred alignment method. For us, this was STAR.
...
### Seperate filtering step to only keep male vehicle DEGs to use as controls
* Used for generating heatmaps where all DEGs (significant or not) were compared to Male Veh
* This will only keep genes that are common between the STAR and Kallisto lists in Male Vehicle condition with log2FC and pAdj values from preferred alignment method. For us, this was STAR.
  + Unlike the ones for all sex/Tx, this step returns a **dataframe** and not a **list**.
...
* These were manually selected using a spreadsheet software like Microsoft Excel or LibreOffice Calc.
* Ones used in Choe2021 are included as .csv files
...
X_ADG <- read_csv("Vignette_materials/X_ADG.csv", col_names = TRUE)
...
### PCA plot of all data. This takes a VERY long time. Not advisable in the vignette, but the script has been provided and commented out. 
* PCA requires:
  + DESeq2 vst pipeline (see original documentation) performed on all (144) samples
  ...
  + Use the ggplot package to generate plots
...
### Make heatmaps where all DEG results (significant or not) are compared to Male Veh control group
* Use DDS_rlog to normalize reads for individual DEG analysis
  ...
  + Repeat for each Sex/Tx as done previously for finding DEGs

```{r Function to run DESeq2 and rlog to normalize the data, only keeping genes with unique gene names, discarding genes with no names & duplicate entries}
...
MalE2_X_DDS_rlog_MV <- readRDS("Vignette_materials/DDS_rlog_MalE2_X_MV.rds")
MalExem_X_DDS_rlog_MV <- readRDS("Vignette_materials/DDS_rlog_MalExem_X_MV.rds")
FemVeh_X_DDS_rlog_MV <- readRDS("Vignette_materials/DDS_rlog_FemVeh_X_MV.rds")
FemE2_X_DDS_rlog_MV <- readRDS("Vignette_materials/DDS_rlog_FemE2_X_MV.rds")
FemExem_X_DDS_rlog_MV <- readRDS("Vignette_materials/DDS_rlog_FemExem_X_MV.rds")
...
* The log2 fold change value range is very big, to make for easier visualization, extreme values should be binned
* Choe2021 set bins at |2| (4x fold change) although other cutoffs are valid
...
### Make Volcano plots using the log2FoldChange and pAdj columns from the common gene lists made above. 
  * Use all genes DE to Sex/Tx/region. **Do not limit this to only use the vehicle male as a standard.**
...
```{r Function to run ClusterProfiler enrichGO function for each gene list of DEGs generated for each group (Sex/Tx/Region) and return the results as a list item}
...
MakeDotplot_list <- function(enrichGO_list){
  for (i in 1:length(enrichGO_list)) {
    ...
    Dotplot <- dotplot(enrichGO_list[[i]], split = "ONTOLOGY", title = Label, showCategory = 8, font.size=15) + 
...
* Make VennEuler plots using the common gene list (GL not GO) made above. 
  ...
  + **DOUBLE CHECK YOUR DATA** example shown here has female veh/exem that do not have any DEGs and are therefore empty lists
...
* Removes DEGs shared with vehicle and exemestane treated females
  ...
  + Steps to run this on LMAN is included as an example (and commented out) since there are no DEGs in the Area X of vehicle and exemestane treated females
    * LMAN data is not included in the vignette, but can be done on your own using the scripts provided in this vignette.
    * The raw Count matrices for STAR and Kallisto aligned data have been provided in directory "Raw_CountMatrices" with a sample ID/library ID key.
```{r}
